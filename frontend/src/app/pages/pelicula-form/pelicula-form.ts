...
export class PeliculaForm implements OnInit {
  ...
  guardar(): void {
    ...
    this.peliculaService.crear(this.formulario.getRawValue()).subscribe({
      next: (pelicula) => this.router.navigate(['/peliculas', pelicula.id]),
      error: () => this.error.set('No se ha podido guardar la pelicula.')
    });
  }
